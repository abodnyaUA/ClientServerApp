...
<head>
	...
	<script type="text/javascript" src="/Site/models.js"></script>
	<script type="text/javascript" src="/JS/addBar.js"></script>
</head>
<body>
	<div id="content">
		<?php if (count($models) > 0) : ?>
		   	<input type="button" value="Add new model" onclick="showBar(); document.getElementById('name').focus();" id="callNewModelPanelButton">
			<table>
				<tbody id="dataTable">
					<tr>
		...
		<?php else : ?>
			<div class="emptyData">
				<p/>There aren't any model. Do you want to add some?<p/>
		   		<input type="button" value="Add new model" onclick="showBar(); document.getElementById('name').focus();" id="callNewModelPanelButton">
			</div>
		<?php endif; ?>
	</div>
	...
	<div id="bar" onload="clearBar()">
	    ...
	    <input type="button" id="addPanelButton" value="Don't add" onclick="hideBar(); clearBar();">
	</div>
</body>
</html>
